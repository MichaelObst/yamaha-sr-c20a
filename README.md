...
This is a python script to interact with the Yamaha SR-C20A soundbar via BLE. It uses Bleak to connect and maintain a BLE connection to the soundbar imitating the "Sound Bar Remote" app from yamaha.
...
Download and extract the code. Copy `config.py.example` to `config.py` and edit the contents. You will need to `pip3 install bleak paho-mqtt flask`. Then run `python main.py`.
